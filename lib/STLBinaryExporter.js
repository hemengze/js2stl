...
THREE.STLBinaryExporter.prototype = {
	...
	parse: ( function () {
		...
		return function parse( scene ) {
			...
			output.writeUInt32LE(triangles, offset); offset += 4;
			...
			objects.forEach( function ( object ) {
				...
				for ( var i = 0, l = faces.length; i < l; i ++ ) {
					...
					output.writeFloatLE(vector.x, offset); offset += 4; // normal
					output.writeFloatLE(vector.y, offset); offset += 4;
					output.writeFloatLE(vector.z, offset); offset += 4;
					...
					for ( var j = 0; j < 3; j ++ ) {
						...
						output.writeFloatLE(vector.x, offset); offset += 4; // vertices
						output.writeFloatLE(vector.y, offset); offset += 4;
						output.writeFloatLE(vector.z, offset); offset += 4;

					}

					output.writeUInt16LE(0, offset); offset += 2; // attribute byte count
